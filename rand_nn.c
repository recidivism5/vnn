...
void randomize_nudges(NeuralNet* netPtr, float learningRate)
{
    unsigned int i;
    unsigned int j;
    ...
    for (i = 1; i < netPtr->numLayers; i++)
    {
        for (j = 0; j < netPtr->layers[i].numNodes; j++)
        {
            netPtr->layers[i].biasNudges[j] = learningRate * random_float(-1.0f, 1.0f);
        }
        for (j = 0; j < netPtr->layers[i].weightMatrixSize; j++)
        {
            netPtr->layers[i].weightNudges[j] = learningRate * random_float(-1.0f, 1.0f);
        }
    }
}

void add_nudges(NeuralNet* netPtr)
{
    unsigned int i;
    unsigned int j;
    ...
    for (i = 1; i < netPtr->numLayers; i++)
    {
        for (j = 0; j < netPtr->layers[i].numNodes; j++)
        {
            netPtr->layers[i].biases[j] += netPtr->layers[i].biasNudges[j];
        }
        for (j = 0; j < netPtr->layers[i].weightMatrixSize; j++)
        {
            netPtr->layers[i].weightMatrix[j] += netPtr->layers[i].weightNudges[j];
        }
    }
}

void sub_nudges(NeuralNet* netPtr)
{
    unsigned int i;
    unsigned int j;
...
void train(NeuralNet* netPtr, TrainingData* tDataPtr, unsigned int batchSize, unsigned int numEpochs, float learningRate)
{
    ...
    avgCost = sumCost / batchSize;
    printf("Epoch %u complete. avgCost = %f\n", epoch+1, avgCost);

    randomize_nudges(netPtr, learningRate);
    add_nudges(netPtr);
    avgCostPrev = avgCost;
